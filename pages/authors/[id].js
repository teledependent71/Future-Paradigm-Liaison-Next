...
import authorsPageInitialPathsF5efdResource from '../../resources/authors-page-initial-paths-f5efd'
import authorsPageInitialProps6f770Resource from '../../resources/authors-page-initial-props-6f770'
  ...
  return (
    <>
      <div className="authors11-container">
        <Head>
          <title>Authors1 - Future Paradigm Liaison</title>
          <meta
            property="og:title"
            content="Authors1 - Future Paradigm Liaison"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsF5efdResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialProps6f770Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
