...
import postsPageInitialPathsCc752Resource from '../../resources/posts-page-initial-paths-cc752'
import postsPageInitialPropsC5579Resource from '../../resources/posts-page-initial-props-c5579'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Future Paradigm Liaison</title>
          <meta
            property="og:title"
            content="Posts1 - Future Paradigm Liaison"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsCc752Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsC5579Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
